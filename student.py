...
class Student:
    def __init__(self, root):
        ...
        # Load image
        #first image
        img = Image.open(r"C:\Users\sharm\OneDrive\Desktop\face_recognition system\college_images\student2.jpeg")
        ...
        #second image
        img1 = Image.open(r"C:\Users\sharm\OneDrive\Desktop\face_recognition system\college_images\student1.jpeg")
        ...
        # third image 
        img2 = Image.open(r"C:\Users\sharm\OneDrive\Desktop\face_recognition system\college_images\student3.jpeg")
        ...
        title_lbl=tk.Label(bg_img,text="STUDENT MANAGEMENT SYSTEM",font=("times new roman",35,"bold"),bg="white",fg="darkgreen")
        ...
        studentName_entry=ttk.Entry(class_Student_frame,textvariable=self.var_phone,width=20,font=("times new roman",13,"bold"))
        studentName_entry.grid(row=3,column=3,padx=5,pady=5,sticky=W)
        ...
        img_right = Image.open(r"C:\Users\sharm\OneDrive\Desktop\face_recognition system\college_images\leftimage.jpeg")
        img_right = img_right.resize((720, 130), Image.Resampling.LANCZOS)  # Corrected resize and method
            ...
            try:
                ...
                if Upadate>0:
                    ...
                    query = """UPDATE student SET 
                        course=%s, Year=%s, Semester=%s, Division=%s, Roll=%s, 
                        Gender=%s, Dob=%s, Email=%s, Phone=%s, Address=%s, Name=%s,
                    ...
                    values = (
                        ...
                        self.var_address.get(),
                        self.var_teacher.get(),
                        self.var_radio1.get(),  # Ensure this value exists
                        self.var_std_id.get(),  # This should match Student_id=%s
                    )
                    ...
                    if not Upadate:
                        return    
                messagebox.showinfo("Sucess","Student detail successfully update completed",parent=self.root)
                conn.commit()
                self.fetch_data()
                conn.close()
            except Exception as es:
        ...
        if self.var_dep.get()=="Select Department" or self.var_std_name.get()=="" or self.var_std_id.get()=="":
            ...
            try:
                ...
                values = (
                    ...
                    self.var_teacher.get(),
                    self.var_radio1.get(),  # Ensure this value exists
                    self.var_std_id.get()==id+1,  # This should match Student_id=%s
                )
                ...
                def face_cropped(img):
                    ...
                    for (x, y, w, h) in faces:
                        ...
                        return face_cropped  # Return the first detected face
                ...
                while True:
                    ...
                    if face is not None:
                        face = cv2.resize(face, (640, 480))  # Resize only if face is detected
                        face = cv2.cvtColor(face, cv2.COLOR_BGR2GRAY)

                        file_name_path = "data/user." + str(id) + "." + str(img_id) + ".jpg"
                        cv2.imwrite(file_name_path, face)

                        cv2.putText(face, str(img_id), (20, 50), cv2.FONT_HERSHEY_COMPLEX, 2, (255, 0, 0), 2)
                        cv2.imshow("Cropped Face", face)

                        img_id += 1  # Increment image ID

                    if cv2.waitKey(1) == 13 or img_id == 100:  # Exit on Enter key or 100 images
                        break
